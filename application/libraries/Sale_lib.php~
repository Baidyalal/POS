...
class Sale_lib
{
	...
	function add_item($item_id,$quantity=1,$discount=0,$price=null,$description=null,$serialnumber=null)
	{
		...
		array(
			...
			'cost_price'=>$price!=null ? $price: $this->CI->Item->get_info($item_id)->cost_price
			)
		);
	...
	function edit_item($line,$description,$serialnumber,$quantity,$discount,$cost_price,$price)
	{
		$items = $this->get_cart();
		if(isset($items[$line]))
		{
	...
	function edit_new_item($line)
	{
	/*	$item_line=$items[$line];
		print "editing new items ....";
		print_r($items, false);*/
	...
	function get_subtotal()
	{
		...
		foreach($this->get_cart() as $item)
		{
			...
			$item_total = $item_total - ($item_total * $item['discount'] / 100); // check for discount
	...
	function get_tax1()
	{
		...
		foreach($this->get_cart() as $line=>$item)
		{
			...
			foreach($tax_info as $tax)
			{
				$tax1 = $item['quantity'] * $item['price'] * $tax['percent'] / 100;
			}
		}
		return to_currency_no_money($tax1);
	}
	...
	function get_tax2()
	{
		...
		foreach($this->get_cart() as $line=>$item)
		{
			...
			foreach($tax_info as $tax)
			{
				$tax2 = $item['quantity'] * $item['price'] * $tax['percent'] / 100;
			}
		}
		return to_currency_no_money($tax2);
	}
