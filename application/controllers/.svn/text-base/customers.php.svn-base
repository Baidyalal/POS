...
class Customers extends Person_controller
{
	...
	*/
	...
	function save($customer_id=-1)
	{
		...
		if($_POST["email"]=='') // if no email, generate email
		{
			$autogen_mail = $_POST["first_name"].$_POST["account_number"]."@pos."."com";
		}
		else    //if mail, do not generate email use the given mailid
		{
			$autogen_mail=$_POST["email"];
		}
		...
		$date= gmdate("Y-m-d");
        	...
        	//To define which kind of user people or employee.
       		$person_type='2';

		$person_data = array(
		'person_type'=>$person_type,
		'update_date'=>$date,
		'person_title'=>$this->input->post('common_title'),
		...
		// account_number in POST uses person_id...change this by prefixing the year
		$year=date('y');
		$autogen_acctno = $year."-"."0000".$_POST["account_number"];

		$customer_data =array(
		'account_number'=>$this->input->post('account_number')=='' ? null : $autogen_acctno,
		'taxable'=>$this->input->post('taxable')=='' ? 0:1,
		);

		if($this->Customer->save($person_data,$customer_data,$customer_id))
		{
                	$sql= mysql_query("select * from ospos_people where email='".$_REQUEST["email"]."'");
                	$num_rows= mysql_num_rows($sql);
                	if($num_rows>0)
                	{
                        	//echo json_encode(array('success'=>true,'message'=>$this->lang->line('customers_mail_valid')));

                        	echo json_encode(array('success'=>false,'message'=>$this->lang->line('customers_mail_valid').' '.
