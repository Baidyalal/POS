...
class Sales extends Secure_area
{
	...
	{
		//to get the item id.
		//$sql="select item_id from ospos_items where item_number = '".$_REQUEST["item_number"]."'";
		$sql="select item_id from ospos_items where item_number = '" . $this->input->post("item_number") . "'";
		$sql_exe=mysql_query($sql);
		$row=mysql_fetch_assoc($sql_exe);
		$item_id= $row["item_id"];
		...
		$quantity = $mode=="sale" ? $this->input->post("item_quantity") : -1;

		//$order_remark = $mode=="sale" ? $this->input->post("description") : -1;
		$description = $this->input->post("category") . " - "  . $this->input->post("description");

		$discount = $mode=="sale" ? $this->input->post("discount") : -1;
		$price = $mode=="sale" ? $this->input->post("unit_price") : -1;
		...
		elseif(!$this->sale_lib->add_item($item_id_or_number_or_item_kit_or_receipt,$quantity,$discount,$price,$description))
		{
	...
	function edit_item($line)
	{
		...
		$cost_price = $this->input->post("price");
		$quantity = $this->input->post("quantity");
		$discount = $this->input->post("discount");
		$price = $this->input->post("unit_price");

		print_r($discount, false);


		if ($this->form_validation->run() != FALSE)
		{
			$this->sale_lib->edit_item($line,$description,$serialnumber,$quantity,$discount,$cost_price,$price);
		}
		else
		{
	...
	function _reload($data=array())
	{
		...
		$data['subtotal']=$this->sale_lib->get_subtotal();
		$data['tax1']=$this->sale_lib->get_tax1();
		$data['tax2']=$this->sale_lib->get_tax2();
		$data['taxes']=$this->sale_lib->get_taxes();
		$data['total']=$this->sale_lib->get_total();
	...
	{
		$data = array();
		$this->load->view('sales/sale_form', $data);
	}

	function edit_new_item($item_number)
	{
		$data = array();
		$data['item_array'] = $this->sale_lib->edit_new_item($item_number);
		$this->load->view('sales/edit',$data);
	}
